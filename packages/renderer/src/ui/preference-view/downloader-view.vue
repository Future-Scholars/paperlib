...
const onAddNewDownloaderClicked = () => {
  ...
  let downloaderPrefs = props.preference.downloaders;
  ...
  window.appInteractor.updatePreference(
    "downloaders",
    JSON.stringify(downloaderPrefs),
    true
  );
  window.appInteractor.recreateDownloaders();
};
...
<template>
  <div class="flex flex-col w-full text-neutral-800 dark:text-neutral-300">
    ...
    <div class="flex flex-col px-2 rounded-md max-h-[450px] overflow-scroll">
      <DownloaderItem
        v-for="(downloaderPref, index) in preference.downloaders"
        :key="index"
