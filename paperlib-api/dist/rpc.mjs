import { c as commonjsGlobal, u as uid } from './misc-BJM5zfKg.mjs';
...
class RendererProcessMessagePortRPCProtocol extends MessagePortRPCProtocol {
  _bindPiniaStates;
  _piniaGroupId;
  constructor(port, callerId, callWithCallerId = false) {
    ...
    this._bindPiniaStates = /* @__PURE__ */ new Map();
    this._piniaGroupId = uid();
  }
  _createProxy(rpcId) {
    const handler = {
      get: (target, name) => {
        if (typeof name === "string" && // TODO: Check: once
        (name === "on" || name === "onChanged" || name === "onClick" || name === "once")) {
          target[name] = (...myArgs) => {
            return this._remoteEventListen(rpcId, myArgs[0], myArgs[1]);
          };
        } else if (typeof name === "string" && (name === "hookModify" || name === "hookTransform") && rpcId === "hookService") {
  ...
  async _bindState(rpcId) {
    ...
    piniaStore.$patchupdate = (patch, direct = false) => {
      const realPatch = {};
      for (const key in patch) {
        if (lodashExports.isEqual(patch[key], piniaStore[key])) {
          continue;
        } else {
...
class RPCService {
  ...
  _remoteAPIs = {};
  _actionors = {};
  async waitForAPI(processID, group, timeout) {
    return new Promise(async (resolve) => {
      for (let i = 0; i < timeout / 100; i++) {
        if (this._protocols[processID] && this._protocols[processID].exposedAPIs[group] && this._protocols[processID].exposedAPIs[group].length > 0) {
          resolve(true);
          break;
        } else {
...
class RendererProcessRPCService extends RPCService {
  constructor(_processID, _exposeAPIGroup) {
