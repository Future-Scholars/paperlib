...
export class CNKIWebImporter extends WebImporter {
  ...
  async parsingProcess(
    ...
    const response = await window.networkTool.postForm(refRequestUrl, refRequestFormData)
    ...
    if (refwork) {
      ...
      if (lines.length === 0) {
        ...
        for (const line of lines) {
          ...
          } else if (line.startsWith('A1') || line.startsWith('A2' || line.startsWith('A3') || line.startsWith('A4'))) {
            const aList = line.slice(3).split(';').map(author => author.trim());
        ...
        paperEntityDraft.setValue('authors', authorList.filter(a => a).join(', '))
