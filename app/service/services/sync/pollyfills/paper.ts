...
export async function toRealmPaperEntity(txOrDb: Transaction, sqlitePaper: SqlitePaper, logger?: LogService): Promise<IEntityObject> {
  ...
  try {
    ...
    const authors = await txOrDb.selectFrom("paperAuthor")
    ...
    const paperTags = await txOrDb.selectFrom("paperTag")
    ...
    const paperFolders = await txOrDb.selectFrom("paperFolder")
    ...
    if (sqlitePaper.feedId) {
      const feedData = await txOrDb.selectFrom("feed")
        .where("id", "=", sqlitePaper.feedId)
        .selectAll()
        .executeTakeFirst();

      if (feedData) {
        feed = await toRealmFeed(txOrDb, feedData);
    ...
    const entity: Entity = new Entity({
      _id: sqlitePaper.legacyOid || new ObjectId(),
