 ...
 * @returns boolean indicating if merge is performed
...
export async function mergePaperFieldLWW(
  ...
  const paperArray = await tx
    .selectFrom('paper')
    .selectAll()
    .where('id', '=', fieldChange.data.id)
    .execute()
  if (paperArray.length > 1) {
    throw new Error(`Multiple papers found for id ${fieldChange.data.id}`)
  }
  const paper = paperArray[0]
  if (!paper) {
    // Paper does not exist, cannot merge, throw error
    throw new SyncCrdtError(`No paper found with id ${fieldChange.data.id}`)
  }
  ...
  if (!version) {
    ...
    throw new SyncCrdtError(`No version found for paper ${fieldChange.data.id} field ${fieldChange.data.field}`)
  }
  
  if (
    (fieldChange.data.value && paper[fieldChange.data.field] === fieldChange.data.value) ||
...
export async function mergeFeedFieldLWW(
  ...
  if (feedArray.length > 1) {
    throw new Error(`Multiple feeds found for id ${fieldChange.data.id}`)
  }
  const feed = feedArray[0]
  
  if (!feed) {
    // Feed does not exist, cannot merge, throw error
    throw new SyncCrdtError(`No feed found with id ${fieldChange.data.id}`)
  }
  ...
  if (!version) {
    // No version exists, cannot merge, throw error
    throw new SyncCrdtError(`No version found for feed ${fieldChange.data.id} field ${fieldChange.data.field}`)
  }
  
  if (
    (fieldChange.data.value && feed[fieldChange.data.field] === fieldChange.data.value) ||
...
export async function mergeAuthorFieldLWW(
  ...
  if (authorArray.length > 1) {
    throw new Error(`Multiple authors found for id ${fieldChange.data.id}`)
  }
  const author = authorArray[0]
  
  if (!author) {
    // Author does not exist, cannot merge, throw error
    throw new SyncCrdtError(`No author found with id ${fieldChange.data.id}`)
  }
  ...
  if (!version) {
    // No version exists, cannot merge, throw error
    throw new SyncCrdtError(`No version found for author ${fieldChange.data.id} field ${fieldChange.data.field}`)
  }
  
  if (
    (fieldChange.data.value && author[fieldChange.data.field] === fieldChange.data.value) ||
...
export async function mergeTagFieldLWW(
  ...
  if (tagArray.length > 1) {
    throw new Error(`Multiple tags found for id ${fieldChange.data.id}`)
  }
  const tag = tagArray[0]
  
  if (!tag) {
    // Tag does not exist, cannot merge, throw error
    throw new SyncCrdtError(`No tag found with id ${fieldChange.data.id}`)
  }
  ...
  if (!version) {
    // No version exists, cannot merge, throw error
    throw new SyncCrdtError(`No version found for tag ${fieldChange.data.id} field ${fieldChange.data.field}`)
  }
  
  if (
    (fieldChange.data.value && tag[fieldChange.data.field] === fieldChange.data.value) ||
...
export async function mergeFolderFieldLWW(
  ...
  if (folderArray.length > 1) {
    throw new Error(`Multiple folders found for id ${fieldChange.data.id}`)
  }
  const folder = folderArray[0]
  
  if (!folder) {
    // Folder does not exist, cannot merge, throw error
    throw new SyncCrdtError(`No folder found with id ${fieldChange.data.id}`)
  }
  ...
  if (!version) {
    // No version exists, cannot merge, throw error
    throw new SyncCrdtError(`No version found for folder ${fieldChange.data.id} field ${fieldChange.data.field}`)
  }
  
  if (
    (fieldChange.data.value && folder[fieldChange.data.field] === fieldChange.data.value) ||
...
export async function mergeSupplementFieldLWW(
  ...
  if (supplementArray.length > 1) {
    throw new Error(`Multiple supplements found for id ${fieldChange.data.id}`)
  }
  const supplement = supplementArray[0]
  
  if (!supplement) {
    // Supplement does not exist, cannot merge, throw error
    throw new SyncCrdtError(`No supplement found with id ${fieldChange.data.id}`)
  }
  ...
  if (!version) {
    // No version exists, cannot merge, throw error
    throw new SyncCrdtError(`No version found for supplement ${fieldChange.data.id} field ${fieldChange.data.field}`)
  }
  
  if (
    (fieldChange.data.value && supplement[fieldChange.data.field] === fieldChange.data.value) ||
